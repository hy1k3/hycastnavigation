...
void duDebugDrawTriMesh(duDebugDraw* dd, const float* verts, int /*nverts*/,
						...
						const uint8_t* flags, const float texScale)
{
	if (!dd) return;
	if (!verts) return;
	if (!tris) return;
	...
	for (int y = 0; y < chf.height; ++y)
	{
		for (int x = 0; x < chf.width; ++x)
		{
			...
			for (unsigned i = c.index, ni = c.index+c.count; i < ni; ++i)
			{
				const rcCompactSpan& s = chf.spans[i];
				const float fy = chf.bmin.y + (s.y+1)*ch;
				const uint8_t cd = (uint8_t)(chf.dist[i] * dscale);
...
void duDebugDrawHeightfieldLayer(duDebugDraw* dd, const struct rcHeightfieldLayer& layer, const int idx)
{
	...
	for (int y = 0; y < h; ++y)
	{
		for (int x = 0; x < w; ++x)
		{
			...
			if (h == 0xff) continue;
			const uint8_t area = layer.areas[lidx];
...
void duDebugDrawLayerContours(duDebugDraw* dd, const struct rcLayerContourSet& lcset)
{
	...
	const Vec3& orig = lcset.bmin;
	const float cs = lcset.cs;
	const float ch = lcset.ch;
	
	const uint8_t a = 255;// (uint8_t)(alpha*255.0f);
...
void duDebugDrawLayerPolyMesh(duDebugDraw* dd, const struct rcLayerPolyMesh& lmesh)
{
	...
	{
		const uint16_t* p = &lmesh.polys[i*nvp*2];
...
static void getContourCenter(const rcContour* cont, const Vec3& orig, float cs, float ch, Vec3& center)
{
	center = Vec3(0, 0, 0);
	if (!cont->nverts)
		return;
...
static const rcContour* findContourFromSet(const rcContourSet& cset, uint16_t reg)
{
...
void duDebugDrawRegionConnections(duDebugDraw* dd, const rcContourSet& cset, const float alpha)
{
	...
	for (int i = 0; i < cset.nconts; ++i)
	{
		...
		for (int j = 0; j < cont->nverts; ++j)
		{
			...
			if (v[3] == 0 || (uint16_t)v[3] < cont->reg) continue;
			const rcContour* cont2 = findContourFromSet(cset, (uint16_t)v[3]);
			if (cont2)
			{
...
void duDebugDrawPolyMesh(duDebugDraw* dd, const struct rcPolyMesh& mesh)
{
	...
	for (int i = 0; i < mesh.npolys; ++i)
	{
		...
		const uint8_t area = mesh.areas[i];
		...
		for (int j = 2; j < nvp; ++j)
		{
			...
			for (int k = 0; k < 3; ++k)
			{
				const uint16_t* v = &mesh.verts[vi[k]*3];
				const float x = orig.x + v[0]*cs;
				const float y = orig.y + (v[1]+1)*ch;
				const float z = orig.z + v[2]*cs;
				dd->vertex(x,y,z, color);
	...
	{
		const uint16_t* v = &mesh.verts[i*3];
		const float x = orig.x + v[0]*cs;
		const float y = orig.y + (v[1]+1)*ch + 0.1f;
		const float z = orig.z + v[2]*cs;
		dd->vertex(x,y,z, colv);
...
void duDebugDrawPolyMeshDetail(duDebugDraw* dd, const struct rcPolyMeshDetail& dmesh)
{
	...
	for (int i = 0; i < dmesh.nmeshes; ++i)
	{
		...
		const Vec3* verts = &dmesh.verts[bverts];
		...
		{
			dd->vertex(verts[tris[j*4+0]].ptr(), color);
			dd->vertex(verts[tris[j*4+1]].ptr(), color);
			dd->vertex(verts[tris[j*4+2]].ptr(), color);
		}
	}
	dd->end();
	...
	for (int i = 0; i < dmesh.nmeshes; ++i)
	{
		...
		for (int j = 0; j < ntris; ++j)
		{
			const uint8_t* t = &tris[j*4];
			for (int k = 0, kp = 2; k < 3; kp=k++)
			{
				uint8_t ef = (t[3] >> (kp*2)) & 0x3;
				if (ef == 0)
				{
					// Internal edge
					if (t[kp] < t[k])
					{
						dd->vertex(verts[t[kp]].ptr(), coli);
						dd->vertex(verts[t[k]].ptr(), coli);
					}
				}
			}
		}
	...
	for (int i = 0; i < dmesh.nmeshes; ++i)
	{
		...
		for (int j = 0; j < ntris; ++j)
		{
			...
			for (int k = 0, kp = 2; k < 3; kp=k++)
			{
				...
				{
					// Ext edge
					dd->vertex(verts[t[kp]].ptr(), cole);
					dd->vertex(verts[t[k]].ptr(), cole);
				}
			}
		}
	}
