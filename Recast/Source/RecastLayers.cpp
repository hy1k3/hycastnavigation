...
static bool contains(const uint8_t* a, const uint8_t an, const uint8_t v)
{
	const int n = (int)an;
	for (int i = 0; i < n; ++i)
	{
...
static bool addUnique(uint8_t* a, uint8_t& an, int anMax, uint8_t v)
{
	if (contains(a, an, v))
		return true;
							  ...
							  rcHeightfieldLayerSet& lset)
{
	...
	rcScopedDelete<uint8_t> srcReg((uint8_t*)rcAlloc(sizeof(uint8_t)*chf.spanCount, RC_ALLOC_TEMP));
	if (!srcReg)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildHeightfieldLayers: Out of memory 'srcReg' (%d).", chf.spanCount);
		return false;
	}
	memset(srcReg,0xff,sizeof(uint8_t)*chf.spanCount);
	...
	const uint16_t mergeHeight = (uint16_t)walkableHeight * 4;
		...
		for (;;)
		{
			...
			for (int j = 0; j < nregs; ++j)
			{
				...
				for (int k = 0; k < nregs; ++k)
				{
					...
					if (contains(ri.layers,ri.nlayers, (uint8_t)k))
					{
						overlap = true;
						break;
					}
	...
	{
		uint8_t curId = (uint8_t)i;

		rcHeightfieldLayer* layer = &lset.layers[i];

		const int gridSize = sizeof(uint8_t)*lw*lh;

		layer->heights = (uint8_t*)rcAlloc(gridSize, RC_ALLOC_PERM);
		if (!layer->heights)
		{
		...
		layer->areas = (uint8_t*)rcAlloc(gridSize, RC_ALLOC_PERM);
		if (!layer->areas)
		{
		...
		layer->cons = (uint8_t*)rcAlloc(gridSize, RC_ALLOC_PERM);
		if (!layer->cons)
		{
		...
		for (int y = 0; y < lh; ++y)
		{
			for (int x = 0; x < lw; ++x)
			{
				...
				for (int j = (int)c.index, nj = (int)(c.index+c.count); j < nj; ++j)
				{
					...
					layer->heights[idx] = (uint8_t)(s.y - hmin);
					...
					for (int dir = 0; dir < 4; ++dir)
					{
						if (rcGetCon(s, dir) != RC_NOT_CONNECTED)
						{
							...
							{
								portal |= (uint8_t)(1<<dir);
								// Update height so that it matches on both sides of the portal.
								const rcCompactSpan& as = chf.spans[ai];
								if (as.y > hmin)
									layer->heights[idx] = rcMax(layer->heights[idx], (uint8_t)(as.y - hmin));
							}
