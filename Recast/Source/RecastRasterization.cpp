...
static void dividePoly(const Vec3* inVerts, int inVertsCount,
                       Vec3* outVerts1, int* outVerts1Count,
                       Vec3* outVerts2, int* outVerts2Count,
...
static bool rasterizeTri(const Vec3& v0, const Vec3& v1, const Vec3& v2,
                         ...
                         const Vec3& heightfieldBBMin, const Vec3& heightfieldBBMax,
...
bool rcRasterizeTriangles(rcContext* context,
                          const TriChunk& chunk, const int numTris, const uint8_t* triAreaIDs,
                          rcHeightfield& heightfield, const int flagMergeThreshold)
{
	...
	for (; i < numTris; ++i)
	{
		...
		if (!overlapBounds(vmin(vmin(a, b), c), vmax(vmax(a, b), c), hfMin, hfMax))
			continue;
		if (!rasterizeTri(a, b, c, triAreaIDs[i], heightfield, hfMin, hfMax,
		                  heightfield.cs, inverseCellSize, inverseCellHeight, flagMergeThreshold))
		{
			context->log(RC_LOG_ERROR, "rcRasterizeTriangles: Out of memory.");
			return false;
