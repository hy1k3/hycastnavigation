...
static uint16_t MESH_NULL_IDX = 0xffff;
...
static int createBVTree(dtNavMeshCreateParams* params, dtBVNode* nodes, int /*nnodes*/)
{
	...
	for (int i = 0; i < params->polyCount; i++)
	{
		...
		if (params->detailMeshes)
		{
			...
			Vec3 bmin = params->detailVerts[vb];
			Vec3 bmax = bmin;

			for (int j = 1; j < ndv; j++)
			{
				bmin = vmin(bmin, params->detailVerts[vb + j]);
				bmax = vmax(bmax, params->detailVerts[vb + j]);
			}

			// BV-tree uses cs for all dimensions
			it.bmin[0] = (uint16_t)dtClamp((int)((bmin.x - params->bmin.x)*quantFactor), 0, 0xffff);
			it.bmin[1] = (uint16_t)dtClamp((int)((bmin.y - params->bmin.y)*quantFactor), 0, 0xffff);
			it.bmin[2] = (uint16_t)dtClamp((int)((bmin.z - params->bmin.z)*quantFactor), 0, 0xffff);

			it.bmax[0] = (uint16_t)dtClamp((int)((bmax.x - params->bmin.x)*quantFactor), 0, 0xffff);
			it.bmax[1] = (uint16_t)dtClamp((int)((bmax.y - params->bmin.y)*quantFactor), 0, 0xffff);
			it.bmax[2] = (uint16_t)dtClamp((int)((bmax.z - params->bmin.z)*quantFactor), 0, 0xffff);
		}
		else
		{
...
static uint8_t classifyOffMeshPoint(const Vec3& pt, const Vec3& bmin, const Vec3& bmax)
{
...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, uint8_t** outData, int* outDataSize)
{
	...
	if (params->offMeshConCount > 0)
	{
		...
		if (params->detailVerts && params->detailVertsCount)
		{
			...
			{
				const float h = params->detailVerts[i].y;
				hmin = dtMin(hmin,h);
				hmax = dtMax(hmax,h);
			}
		}
		else
		{
			for (int i = 0; i < params->vertCount; ++i)
			{
				...
				const float h = params->bmin.y + iv[1] * params->ch;
				hmin = dtMin(hmin,h);
				hmax = dtMax(hmax,h);
			}
		}
		...
		Vec3 bmin = params->bmin;
		Vec3 bmax = params->bmax;
		bmin.y = hmin;
		bmax.y = hmax;

		for (int i = 0; i < params->offMeshConCount; ++i)
		{
			const Vec3& p0 = params->offMeshConVerts[i*2+0];
			const Vec3& p1 = params->offMeshConVerts[i*2+1];
	...
	const int vertsSize = dtAlign4(sizeof(Vec3)*totVertCount);
	const int polysSize = dtAlign4(sizeof(dtPoly)*totPolyCount);
	const int linksSize = dtAlign4(sizeof(dtLink)*maxLinkCount);
	const int detailMeshesSize = dtAlign4(sizeof(dtPolyDetail)*params->polyCount);
	const int detailVertsSize = dtAlign4(sizeof(Vec3)*uniqueDetailVertCount);
	...
	Vec3* navVerts = dtGetThenAdvanceBufferPointer<Vec3>(d, vertsSize);
	...
	Vec3* navDVerts = dtGetThenAdvanceBufferPointer<Vec3>(d, detailVertsSize);
	...
	{
		const uint16_t* iv = &params->verts[i*3];
		navVerts[i].x = params->bmin.x + iv[0] * params->cs;
		navVerts[i].y = params->bmin.y + iv[1] * params->ch;
		navVerts[i].z = params->bmin.z + iv[2] * params->cs;
	}
	...
	for (int i = 0; i < params->offMeshConCount; ++i)
	{
		...
		if (offMeshConClass[i*2+0] == 0xff)
		{
			navVerts[offMeshVertsBase + n*2+0] = params->offMeshConVerts[i*2+0];
			navVerts[offMeshVertsBase + n*2+1] = params->offMeshConVerts[i*2+1];
			n++;
		}
	}
	...
	if (params->detailMeshes)
	{
		...
		for (int i = 0; i < params->polyCount; ++i)
		{
			...
			dtl.vertBase = (uint32_t)vbase;
			dtl.vertCount = (uint8_t)(ndv-nv);
			dtl.triBase = (uint32_t)params->detailMeshes[i*4+2];
			dtl.triCount = (uint8_t)params->detailMeshes[i*4+3];
			// Copy vertices except the first 'nv' verts which are equal to nav poly verts.
			if (ndv-nv)
			{
				memcpy(&navDVerts[vbase], &params->detailVerts[vb+nv], sizeof(Vec3)*(ndv-nv));
	...
	for (int i = 0; i < params->offMeshConCount; ++i)
	{
		...
		if (offMeshConClass[i*2+0] == 0xff)
		{
			...
			con->pos[0] = params->offMeshConVerts[i*2+0];
			con->pos[1] = params->offMeshConVerts[i*2+1];
...
bool dtNavMeshHeaderSwapEndian(uint8_t* data, const int /*dataSize*/)
{
	...
	// bmin and bmax are Vec3 = 3 floats each; swap each float
...
bool dtNavMeshDataSwapEndian(uint8_t* data, const int /*dataSize*/)
{
	...
	// Vertices (Vec3 = 3 floats, treat as flat float array for endian swap)
	...
	// Detail verts (Vec3 = 3 floats, treat as flat float array)
	...
	for (int i = 0; i < header->offMeshConCount; ++i)
	{
		...
		// pos[2] is Vec3[2] = 6 floats; treat as flat float array
		float* posFloats = &con->pos[0].x;
		for (int j = 0; j < 6; ++j)
			dtSwapEndian(&posFloats[j]);
